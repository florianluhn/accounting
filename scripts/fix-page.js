...
// 1. Remove the stray block around line 624
const strayRegex = /\s*\{\/each\}\s*\{\/each\}\s*<\/div>\s*<\/div>\s*<\/div>\s*\{\/if\}\s*(?=\{:else\})/g;
// Wait, the stray block is before `{:else}`? 
// Let's look at line 630: `{:else}`. Yes! 
content = content.replace(strayRegex, '');

// 2. Remove the truncated block at the bottom (lines 904-951)
// It starts with `<!-- Budget vs Actual Graphs -->` and ends before `{:else}`
const truncRegex = /\s*<!-- Budget vs Actual Graphs -->[\s\S]*?(?=\s*\{:else\})/g;
content = content.replace(truncRegex, '');

// 3. Now the file should be clean of the chart.
// Let's append the correct full chart before the `{:else}` of Profit Loss.
// The `{:else}` of Profit Loss is the one right after the Net Income block.
// Let's find `<!-- Net Income -->` block ending.
