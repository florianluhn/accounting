 ...
 */
function diffMonths(aYear, aMonth, bYear, bMonth) {
 ...
 */
function firstYearConvention(convention, placedInServiceMonth // 1-indexed
) {
    switch (convention) {
        case 'half_year':
 ...
 */
function computeAnnualDepreciations(method, cost, salvageValue, usefulLifeYears) {
    const depreciableBase = cost - salvageValue;
    if (depreciableBase <= 0 || usefulLifeYears <= 0)
        return [];
    if (method === 'SL') {
 ...
 */
export function generateSchedule(method, convention, cost, salvageValue, usefulLifeMonths, activationDate // ISO date 'YYYY-MM-DD'
) {
    const depreciableBase = cost - salvageValue;
    // Edge cases
    if (depreciableBase <= 0 || usefulLifeMonths <= 0 || cost <= 0)
        return [];
 ...
 */
export function calculateMonthlyDepreciation(method, convention, cost, salvageValue, usefulLifeMonths, activationDate, targetMonth, // 'YYYY-MM'
accumulatedSoFar) {
    const depreciableBase = cost - salvageValue;
    if (depreciableBase <= 0 || usefulLifeMonths <= 0 || cost <= 0)
        return 0;
    if (accumulatedSoFar >= depreciableBase)
        return 0;
    const { year: startYear, month: startMonth } = parseISODate(activationDate);
 ...
 * `alreadyPostedMonths`.
 */
export function getEligibleMonths(activationDate, usefulLifeMonths, throughMonth, alreadyPostedMonths) {
